...
export async function ablup39(
  ctx: TrainingContext,
  character: Character
): Promise<void> {
  ctx.drawLine();
  if (ctx.abilities[39] >= 5 && ctx.talents[231] === 0) {
    ctx.showMessage('이 이상 올리려면 소질이 필요합니다'); ctx.waitInput();
    return 0;
  } else if (ctx.abilities[39] >= 10) {
  ...
  ctx.showMessage(`${ctx.getVarName("ABL", 11)}${ctx.abilities[39] + 1}LV이상(현재${ctx.abilities[11]})그리고`);
  ...
  ctx.showMessage(`　　　${ctx.getVarName("EXP", 54)}　${ctx.exp[54]}/{C}`);
  ...
  ctx.showMessage(`${ctx.getVarName("ABL", 39)}의 레벨이 %조사처리(ABL:39,"가")% 되었습니다`);
  return 0;
}

export async function decide_ablup39(
  ctx: TrainingContext,
  character: Character
): Promise<void> {
  if (ctx.abilities[39] >= 10) {
    return 0;
  }
  if (ctx.abilities[39] >= 5 && ctx.talents[231] == 0) {
    return 0;
  ...
  if (ctx.abilities[39] === 0) {
    ...
    B = 3000000;
    C = 250;
  } else if (ctx.abilities[39] === 7) {
    A = 120000;
    B = 500000;
    C = 350;
  } else if (ctx.abilities[39] === 8) {
    A = 200000;
    B = 800000;
    C = 500;
    F = 5;
  } else if (ctx.abilities[39] === 9) {
    A = 500000;
    B = 1500000;
    C = 600;
    F = 7;
  ...
  F = 0;
  if (ctx.abilities[39] >= 2 && (ctx.talents[72] == 0 && ctx.talents[76] == 0)) {
    F = ctx.abilities[39] + 1;
  }
  if (ctx.talents[20]) {
    ctx.times('A', 2.50);
    ctx.times('B', 2.50);
  ...
  if (ctx.abilities[11] < ctx.abilities[39] + 1) {
    I |= 4;
  }
  if (ctx.juel[14] < A) {
    I |= 1;
