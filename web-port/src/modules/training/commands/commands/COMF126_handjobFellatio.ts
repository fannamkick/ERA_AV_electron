// 원본: ERB/指導関係/COMF126.ERB (전체 553라인)
// 수음펠라 - 수음(handjob) + 펠라치오 조합

import { CommandPlugin } from '../../types';
import { TrainingContext, SourceValues } from '../../types';

export const comf126HandjobFellatio: CommandPlugin = {
  id: 126,
  name: '수음펠라',
  category: '봉사',
  staminaCost: 0,

  // 원본: COMF126.ERB 라인 8-219
  ...
  // 원본: COMF126.ERB 라인 221-553
  async execute(context: TrainingContext): Promise<void> {
    context.selectCom = 126;
    context.showMessage('수음펠라');

    // TRAIN_MESSAGE_B (라인 221-222)
    context.saveStrings[0] = '수음펠라';
  ...
  async calculateEjaculationGauge(context: TrainingContext): Promise<void> {
    ...
    // TALENT:혀놀림をみる (라인 361-363)
    if (context.talents[52]) {
      B = Math.floor(B * 2.00);
    }

    // プレイヤーのABL:C감각をみる (라인 365-378)
  ...
  async generateMessage(context: TrainingContext): Promise<void> {
    context.showMessage('수음과 펠라치오를 동시에 수행한다.');
  },
