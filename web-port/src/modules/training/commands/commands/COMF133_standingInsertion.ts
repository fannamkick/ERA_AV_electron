/**
 * COMF133: 서서삽입 (Standing Insertion)
 * 원본: ERB/指導関係/COMF133.ERB
 *
 * セックス系コマンド: 후배위と가슴애무とキスの合わせ技
 */

import { CommandPlugin } from '../../types';
import { TrainingContext, SourceValues } from '../../types';

export const comf133StandingInsertion: CommandPlugin = {
  id: 133,
  name: '서서삽입',
  category: '삽입',
  staminaCost: 25,
  ...
  async execute(context: TrainingContext): Promise<void> {
    ...
    // PRINTL 서서삽입 (라인 16)
    context.showMessage('서서삽입');

    // SAVESTR:0 = 서서삽입 (라인 17)
    context.saveStr = context.saveStr || [];
    context.saveStr[0] = '서서삽입';
