/**
 * COMF71 - 더블 파이즈리 (Double Paizuri / Titjob)
 *
 * Target and assistant perform paizuri on the trainer simultaneously.
 * Similar to double blowjob but focuses on breast stimulation.
 * Includes incest detection and relationship prefix display.
 * Features semen sharing mechanics if either has semen addiction.
 */

import type { CommandPlugin, TrainingContext } from '../../types';

export const COMF71: CommandPlugin = {
  id: 71,
  name: '더블 파이즈리',
  category: 'assistant',
  ...
  isAvailable(context: TrainingContext): boolean {
    ...
    let dirtPenalty = 0;
    if (player.stains.penis & 1) dirtPenalty += 1; // Love juice
    if (player.stains.penis & 4) dirtPenalty += 3; // Semen
    if (player.stains.penis & 8) dirtPenalty += 7; // Anal
    if (player.stains.penis & 16) dirtPenalty += 1; // Breast milk
    if (player.stains.body & 32) dirtPenalty += 3; // Urine
  ...
  async execute(state: GameState, target: Character, player: Character): Promise<void> {
    ...
    // ABL:B감각
    let breastSource = 0;
    if (target.abl.B감각 === 0) breastSource = 100;
    else if (target.abl.B감각 === 1) breastSource = 200;
    else if (target.abl.B감각 === 2) breastSource = 400;
    else if (target.abl.B감각 === 3) breastSource = 800;
    else if (target.abl.B감각 === 4) breastSource = 1200;
    else breastSource = 1500;

    // TALENT:거유
    if (target.talent.거유) breastSource *= 1.20;

    // TALENT:B민감 / B둔감
    if (target.talent.B민감) breastSource *= 1.20;
    else if (target.talent.B둔감) breastSource *= 0.70;

    source[17] = (source[17] || 0) + breastSource;
    ...
    // Assistant's ABL:B감각
    if (assistant) {
      if (assistant.abl.B감각 === 0) {
        source[4] *= 0.80;
        source[5] *= 0.50;
      } else if (assistant.abl.B감각 === 1) {
        source[4] *= 0.90;
        source[5] *= 0.70;
      } else if (assistant.abl.B감각 === 2) {
        source[4] *= 1.00;
        source[5] *= 1.00;
      } else if (assistant.abl.B감각 === 3) {
        source[4] *= 1.10;
        source[5] *= 1.20;
      } else if (assistant.abl.B감각 === 4) {
        source[4] *= 1.20;
        source[5] *= 1.40;
      } else {
        source[4] *= 1.30;
    ...
    else if (target.abl.기교 === 1) ejacGauge = 1500;
    else if (target.abl.기교 === 2) ejacGauge = 2000;
    else if (target.abl.기교 === 3) ejacGauge = 2500;
    else if (target.abl.기교 === 4) ejacGauge = 3000;
    else ejacGauge = 4000;
    ...
    if (ejacLevel === 2) {
      ...
      target.experience.레즈 = (target.experience.레즈 || 0) + 6;
    ...
    // Stain transfers - breasts <-> penis
    target.stains.breast |= player.stains.penis;
    player.stains.penis |= target.stains.breast;

    if (assistant) {
      assistant.stains.breast |= player.stains.penis;
      player.stains.penis |= assistant.stains.breast;
    }

    // Clean up if high skill (also does fellatio as part of paizuri)
    ...
    if (assistant && !target.talent.オトコ && !assistant.talents.オトコ &&
        (target.abl.정액중독 >= 1 || assistant.abl.정액중독 >= 1)) {
    ...
    if (assistant) {
      if (!target.talent.オトコ && !assistant.talents.オトコ) {
        target.experience.레즈 = (target.experience.레즈 || 0) + 5;
      } else if (target.talent.オトコ && assistant.talents.オトコ) {
        target.experience.호모 = (target.experience.호모 || 0) + 5;
      }
    }

    // Trainer is futa
    if (player.talents.후타나리) {
      source[21] = Math.floor((source[21] || 0) / 2);
    }
