...
export async function ablup41(
  ctx: TrainingContext,
  character: Character
): Promise<void> {
  ctx.drawLine();
  if (ctx.abilities[41] >= 5 && ctx.talents[137] === 0) {
  ...
  C = 0;
  F = 0;
  await decide_ablup41(ctx, character);
  if (F > 0) {
    ctx.showMessage(`${ctx.getVarName("EXP", 50)}{F}이상(현재${ctx.exp[50]})그리고`);
  }
  if (ctx.talents[137]) {
    ctx.showMessage(`${ctx.getVarName("ABL", 10)}${ctx.abilities[41] + 1}LV이상(현재${ctx.abilities[10]}LV) 그리고`);
  } else {
    ctx.showMessage(`${ctx.getVarName("ABL", 11)}${ctx.abilities[41] + 1}LV이상(현재${ctx.abilities[11]}LV) 그리고`);
  }
  ...
  ctx.showMessage(`　　　${ctx.getVarName("EXP", 55)}　${ctx.exp[55]}/{C}`);
  ...
  ctx.showMessage(`${ctx.getVarName("ABL", 41)}의 레벨이 %조사처리(ABL:41,"가")% 되었습니다`);
  return 0;
}

export async function decide_ablup41(
  ctx: TrainingContext,
  character: Character
): Promise<void> {
  if (ctx.abilities[41] == 10) {
    return 0;
  }
  if (ctx.abilities[41] >= 5 && ctx.talents[137] == 0) {
    return 0;
  ...
  F = 0;
  if (ctx.abilities[41] >= 2 && (ctx.talents[72] == 0 && ctx.talents[76] == 0 && ctx.talents[137] == 0)) {
    F = ctx.abilities[41] + 1;
  }
  if (ctx.talents[20]) {
    ctx.times('A', 2.50);
    ctx.times('B', 2.50);
  ...
  if (ctx.talents[137]) {
    if (ctx.abilities[10] < ctx.abilities[41] + 1) {
      I |= 4;
    }
    if (ctx.juel[5] < A) {
      I |= 1;
    ...
    if (ctx.abilities[11] < ctx.abilities[41] + 1) {
      I |= 4;
    }
    if (ctx.juel[5] < A) {
      I |= 1;
