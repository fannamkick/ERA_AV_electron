...
const commandInfo = {
  ...
  30: { name: '수음', desc: '대상이 조교자 페니스를 손으로 자극' },
  ...
  64: { name: '3P', desc: '특수 - 조교자와 조수가 동시에 두 곳 공격' },
  ...
  72: { name: '행복초(미약)', desc: '도구 - 미약' },
  73: { name: '유두 맞대기', desc: '조수계 - 유두 맞대기 (미완성)' },
  74: { name: '조합커맨드14', desc: '조합커맨드 (미구현)' },
  75: { name: '조합커맨드15', desc: '조합커맨드 (미구현)' },
  76: { name: '조합커맨드16', desc: '조합커맨드 (미구현)' },
  77: { name: '조합커맨드17', desc: '조합커맨드 (미구현)' },
  78: { name: '조합커맨드18', desc: '조합커맨드 (미구현)' },
  79: { name: '조합커맨드19', desc: '조합커맨드 (미구현)' },
  ...
  86: { name: '조합커맨드26', desc: '조합커맨드 (미구현)' },
...
// 각 커맨드 클래스에 주석 추가 및 getName() 수정
for (let i = 0; i <= 92; i++) {
  const info = commandInfo[i];
  if (!info) continue;

  // 클래스 선언 앞에 주석 추가
  const classPattern = new RegExp(`(export class Com${i}Command extends TrainingCommand \\{)`, 'g');
  const replacement = `/**
 * COM${i}: ${info.name}
 * ${info.desc}
 */
$1`;
  ...
  // getName() 메서드의 반환값 수정
  const getNamePattern = new RegExp(
    `(class Com${i}Command[\\s\\S]{0,500}?getName\\(\\): string \\{[\\s\\n\\r\\t ]{0,50}return ')[^']+('`,
    'gm'
  );
  content = content.replace(getNamePattern, `$1${info.name}$2`);
}

// 파일 저장
fs.writeFileSync(improvedPath, content, 'utf-8');
console.log('주석 및 커맨드 이름 업데이트 완료!');
console.log(`업데이트된 커맨드: ${Object.keys(commandInfo).length}개`);
