...
function hasBlockingConflict(draft: CommandDraft): boolean {
  return draft.unresolvedConflicts.some((conflict) => {
...
export function materializeApprovalCandidate(options: MaterializeOptions): {
  ...
  if (!result.localValidation.ok || result.localValidation.errors.length > 0) {
    throw new Error(`Refusing to materialize result with local validation errors:\n${result.localValidation.errors.join('\n')}`);
  }
  if (!result.draftPath) {
  ...
  if (hasBlockingConflict(draft)) {
    throw new Error('Refusing to materialize draft with blocking unresolved conflicts.');
  }
