...
export async function ablup72(
  ctx: TrainingContext,
  character: Character
): Promise<void> {
  ctx.drawLine();
  if (ctx.abilities[72] >= 5 && (ctx.talents[76] === 0 && ctx.talents[85] === 0)) {
    ctx.showMessage('이 이상 올리려면 소질이 필요합니다'); ctx.waitInput();
    return 0;
  } else if (ctx.abilities[72] >= 10) {
  ...
  ctx.showMessage('');
  ctx.showMessage(`　　　${ctx.getVarName("EXP", 72)}　${ctx.exp[72]}/{B}`);
  ...
  ctx.abilities[72] += 1;
  ctx.juel[7] -= A;
  ctx.showMessage(`${ctx.getVarName("ABL", 72)}의 레벨이 %조사처리(ABL:72,"가")% 되었습니다`);
  return 0;
}

export async function decide_ablup72(
  ctx: TrainingContext,
  character: Character
): Promise<void> {
  if (ctx.abilities[72] >= 10) {
    return 0;
  }
  if (ctx.abilities[72] >= 5 && ctx.talents[76] == 0 && ctx.talents[85] == 0) {
    return 0;
  ...
  B = 0;
  I = 0;
  if (ctx.abilities[72] === 0) {
    A = 75;
    B = 5;
  } else if (ctx.abilities[72] === 1) {
    A = 500;
    B = 15;
  } else if (ctx.abilities[72] === 2) {
    A = 3000;
    B = 30;
  } else if (ctx.abilities[72] === 3) {
    A = 9000;
    B = 75;
  } else if (ctx.abilities[72] === 4) {
    A = 24000;
    B = 120;
  } else if (ctx.abilities[72] === 5) {
    A = 30000;
    B = 180;
  } else if (ctx.abilities[72] === 6) {
    A = 35000;
    B = 400;
  } else if (ctx.abilities[72] === 7) {
    A = 40000;
    B = 600;
  } else if (ctx.abilities[72] === 8) {
    A = 45000;
    B = 800;
  } else if (ctx.abilities[72] === 9) {
    A = 50000;
    B = 1500;
  }
  if (ctx.talents[50]) {
