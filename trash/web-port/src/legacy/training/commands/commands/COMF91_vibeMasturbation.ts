/**
 * COMF91 - 전동마사지기자위 (Electric Massager Masturbation)
 *
 * Similar to rotor masturbation but with an electric massager (more intense).
 * Essentially the same as COMF90 but with slightly higher base values.
 * Can combine with vibrator insertion, shower, or public settings.
 */

import type { CommandPlugin, TrainingContext } from '../../types';

export const COMF91: CommandPlugin = {
  id: 91,
  name: '전동마사지기자위',
  ...
  isAvailable(context: TrainingContext): boolean {
    ...
    let threshold = 37; // Slightly higher than rotor
  ...
  async execute(state: GameState, target: Character, player: Character): Promise<void> {
    state.saveStr[0] = '전동마사지기자위';

    // LOSEBASE (more intense than rotor)
    state.loseBase.health += 30;
    state.loseBase.stamina += 130;
    ...
    // ABL:C감각 - higher base values than rotor
    if (target.abl.C감각 === 0) {
      source[0] = 1015;
      source[12] = 2000;
      source[13] = 500;
    ...
    // ABL:B감각
    if (target.abl.B감각 === 0) source[17] = 1015;
    else if (target.abl.B감각 === 1) source[17] = 1300;
    else if (target.abl.B감각 === 2) source[17] = 1800;
    else if (target.abl.B감각 === 3) source[17] = 2350;
    else if (target.abl.B감각 === 4) source[17] = 2900;
    else source[17] = 3400;
