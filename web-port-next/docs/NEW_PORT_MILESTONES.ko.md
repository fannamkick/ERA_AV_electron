...
- [ ] 소유 범위 안의 모든 원본 row, feature, definition, save address, session/calculation address를 구현, mapping, approved-excluded, blocker 중 하나로 닫는다.
- [ ] blocker가 남으면 그 blocker가 막는 완료 범위를 명시하고, 해당 마일스톤은 완료 체크하지 않는다.
