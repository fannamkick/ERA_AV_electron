...
| M29 | 구매형 상점 listing과 구매 결과 | strict 재판정 결과 blocked. source 206 중 implemented-verified 83, approved-excluded 105, blocked 18. 즉시 사용 item 효과는 M30이지만 item 30/31/38/39/40/41/42/43/52의 listing/ITEMSALES/구매 선택은 M29 책임이다. | 비구매 item/use/equipment/recruit/event row를 M29 완료로 세지 않는다. 단 purchasable immediate-use item listing을 M30으로 넘기면 안 된다. |
...
| M31 | 영입 listing과 인물 생성 결과 | strict 재판정 결과 blocked. source 237 중 implemented-verified 127, approved-excluded 19, blocked 91. downstream field semantics는 M32/M33/M34/M35가 소유해도 recruit creation result integration은 M31 책임이다. | 생성 결과 initial identity/body/social/time 적용 검증을 downstream owner 이름만 보고 넘기면 안 된다. |
...
| M47 | 세계/이벤트/story | 아직 구현 전. strict manifest 기준 total 10, blocked 10. M28/M29/M31/M32에서 넘어온 event/world 책임이 포함된다. | 엔딩/meta 저장은 M48 책임이다. |
...
- M29 is not complete under the source-unit manifest rule.
...
- `data/coverage/milestones/M29-closure.json`: status `blocked`, ownedTotal 101, implemented 83, ownedBlocker 18.
- The 18 blocked rows are item 30/31/38/39/40/41/42/43/52 definition and `ITEMSALES` rows. Effects belong to M30, but purchase listing/visibility/selection belongs to M29.
- 105 approved-excluded rows are explicit inbound responsibility in receiver manifests. Newly added blocked inbound rows: M32 4, M37 2, M47 4, M48 1, M49 1. The 18 reclaimed listing/ITEMSALES rows stay M29-owned blocked.
...
- strict 완료 전에는 previous completed 기록이나 receiver owner 이름을 완료 근거로 쓰지 않는다.
...
- [ ] `data/coverage/milestones/M29-closure.json` source-owner reclaim correction 적용. sourceUnitTotal 206, ownedTotal 101, implemented 83, approvedExcludedFromM29 105, ownedBlocker 18. 현재 status는 `blocked`이며 완료가 아니다.
...
- 2026-05-06 source-owner reclaim audit에서 M31 source 237개는 implemented-verified 127, approved-excluded 19, blocked 91로 정정했다.
- blocked 91개는 recruit creation result integration 83개와 invalid self-exclusion/source-review 8개다. downstream field semantics는 M32/M33/M34/M35가 닫더라도, recruit creation이 initial fields를 생성 결과에 적용하는 검증은 M31 책임이다.
- `npm run gate:milestone-scope-closure -- M31`은 status `blocked` 때문에 실패해야 한다.
