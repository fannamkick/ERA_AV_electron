...
| M33 | 신체/능력/소질/경험 seed와 표시/저장 owner | 기존 구현 이력은 있으나 strict manifest 기준 total 5,372, implemented 4,768, blocked 537, redesign 67. M32에서 넘어온 TALENT save field 4개가 blocked inbound다. | 업무/촬영/훈련 결과 계산은 M37/M39/M42~M44 책임이다. mapped-only 정의 row는 완료가 아니다. |
| M34 | 관계/CFLAG/장비/의복 owner | 기존 구현 이력은 있으나 strict manifest 기준 total 2,238, implemented 1,998, blocked 240. CFLAG mapped-only row를 BLOCKED로 표기했다. | 의미 불명 CFLAG, event/training/mission 효과는 M34 완료가 아니다. |
...
- [VERIFY:M33] [x] Chara seed의 `BASE`, `ABL`, `TALENT`, `EXP`, `MARK`, `PALAM` 계열을 owner별로 연결. strict manifest의 mapped-only 정의 row 533개는 직접 consumer/verification으로 승격 전 완료 금지다.
...
- [BLOCKED:M33] [x] M20/M24 coverage의 body 관련 status 갱신. strict manifest의 533 blocked와 67 redesign이 닫히기 전에는 완료 금지다.
...
- [VERIFY:M34] [x] CFLAG 정의 151개와 Chara CFLAG seed 1,465개를 의미별 owner로 분해. strict manifest의 mapped-only CFLAG row 240개는 직접 owner evidence로 승격 전 완료 금지다.
...
- [BLOCKED:M34] [x] M20/M24/M25 coverage의 관련 status 갱신. strict manifest의 240 blocked가 닫히기 전에는 완료 금지다.
