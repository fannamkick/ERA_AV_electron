...
| M35 | 턴 종료/시간/hook/session cleanup | 기존 통과 이력은 있으나 strict manifest 기준 total 8, implemented 0, blocked 7, redesign 1. hook 호출과 내부 효과 owner를 분리했다. | 업무/미션/이벤트/판매/보상 내부 효과를 M35 완료로 세지 않는다. |
...
- [BLOCKED:M35] [x] 턴 종료 전 hook, 턴 종료 후 hook, 월말 hook, 새 주 hook을 정의
...
- [BLOCKED:M35] [x] 턴 종료 중 session 선택값이 남지 않는지 검증
- [BLOCKED:M35] [x] 턴 종료 후 저장 roundtrip을 검증
- [BLOCKED:M35] [x] M19/M24/M25 coverage의 turn-end 관련 status 갱신
