...
| M35 | 턴 종료/시간/hook/session cleanup | strict closure 완료. 8개 functional turn pipeline 단위는 implemented-verified이고, 기존 7개 save-field mapped row는 supporting evidence로만 남겼다. | 업무/미션/이벤트/판매/보상 내부 효과를 M35 완료로 세지 않는다. M36~M41은 아직 각자 닫아야 한다. |
| M36 | 방문/시설 | 기존 구현 이력은 있으나 strict manifest 기준 total 93, implemented 86, blocked 7. 방문 action과 world/event 효과를 분리했다. | world/event 진행 효과는 M47 책임이다. |
...
- [BLOCKED:M36] [x] 방문 장소 전체를 장소 정의 또는 사용자 승인 제외로 분류하고, 미판정 blocker가 있으면 완료하지 않음
...
- [BLOCKED:M36] [x] M19/M21/M24/M25 coverage의 방문 관련 status 갱신
