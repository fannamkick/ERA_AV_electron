...
| M32 | 인물 원형과 identity/lifecycle | 기존 구현 이력은 있으나 strict manifest 기준 total 298, implemented 286, blocked 12. 체크 항목에 HERE/VERIFY/BLOCKED를 직접 표기했다. | M29 inbound TALENT 4개, 성적 이력/직업 csv-row 5개, source-file-review 3개는 M32에서 다시 닫아야 한다. |
| M33 | 신체/능력/소질/경험 seed와 표시/저장 owner | 기존 구현 이력은 있으나 strict manifest 기준 total 5,368, implemented 4,768, blocked 533, redesign 67. 효과 계산 owner는 분리 표기했다. | 업무/촬영/훈련 결과 계산은 M37/M39/M42~M44 책임이다. mapped-only 정의 row는 완료가 아니다. |
...
| M47 | 세계/이벤트/story | 아직 구현 전. strict manifest 기준 total 8, blocked 8. M28/M29/M31에서 넘어온 event/world 책임이 포함된다. | 엔딩/meta 저장은 M48 책임이다. |
| M48 | 엔딩/계승/global meta | 아직 구현 전. strict manifest 기준 total 3, blocked 2, redesign 1. | 정보/설정 UI는 M49, 전체 저장 검증은 M50 책임이다. |
| M49 | 정보/도움말/설정/남은 기능 | 아직 구현 전. strict manifest 기준 total 3, blocked 2, redesign 1. | catch-all로 남은 기능을 숨기면 안 된다. intake 없는 기능은 원 owner로 되돌린다. |
...
- [VERIFY:M32] [x] 삭제, 은퇴, 조수 가능, 영입 가능 상태를 저장 필드로 분리. M29 inbound TALENT 4개와 성적 이력/직업 identity csv-row 5개는 strict manifest에서 아직 blocked이므로 M32 재판정 때 직접 닫아야 한다.
- [VERIFY:M32] [x] 캐릭터 정보 화면에서 identity 값 표시를 검증
- [VERIFY:M32] [x] 원형에서 인스턴스 생성 후 저장 roundtrip을 검증
- [BLOCKED:M32] [x] Chara seed 중 identity 관련 row의 coverage status 갱신. `zname`, `c_club_girlname`, `ntr_boyfriendname_calc` source-file-review 3개는 라벨/row 단위로 분해 전 완료 금지다.
