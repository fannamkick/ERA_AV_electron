...
| M34 | relationship/CFLAG/equipment/clothing owner | strict closure complete. total 2,247 implemented-verified, approved-excluded 0, blocked 0. M33 CFLAG/FLAG/PBAND 79 rows and item 211 apron behavior are closed by M34 evidence. | Event/training/mission effects remain later-owner responsibilities. mapped/source-file-review is not M34 completion. |
...
| M | 원래 맡은 일 | 실제 처리 / 현재 판정 | 완료로 세면 안 되는 것 / 다음 책임 |
| --- | --- | --- | --- |
| M28 | 메인 route/action/view 전수 연결 | strict closure 완료. source 27 중 M28-owned 24는 implemented-verified, BOYFRIEND event-local 3개는 approved-excluded 및 M47 책임. | event/world 진행 결과는 M28 완료가 아니다. |
| M29 | 구매형 상점 listing과 구매 결과 | strict closure 완료. source 206 중 M29-owned implemented-verified 101, approved-excluded 105, blocked 0. 즉시 사용 item 효과는 M30이지만 item 30/31/38/39/40/41/42/43/52의 listing/ITEMSALES/구매 선택은 M29 책임으로 회수해 구현 검증했다. | 비구매 item/use/equipment/recruit/event row를 M29 완료로 세지 않는다. purchasable immediate-use item listing은 M29가 닫았다. |
| M30 | 즉시 사용 아이템 flow/effect | strict closure 완료. source 74 중 M30-owned 37은 implemented-verified, 특수/훈련/의복/availability 37개는 M34-closed plus M41/M42/M43/M44 수신 책임. | 특수 item 200~214와 훈련 효과는 M30 완료가 아니다. |
| M31 | 영입 listing과 인물 생성 결과 | strict closure 완료. source 237 중 M31-owned implemented-verified 153, approved-excluded 84, blocked 0. downstream field semantics는 M32/M33/M34/M35가 소유해도 recruit creation result integration은 M31 책임으로 회수해 구현 검증했다. | 생성 결과 initial identity/body/social/time 적용 검증을 downstream owner 이름만 보고 넘기면 안 된다. |
| M32 | 인물 원형과 identity/lifecycle | strict closure 완료. source 298 중 M32-owned 291은 implemented-verified, 7개는 approved-excluded 및 M33/M47/M49 수신 책임. | TALENT save field, event generated name, Korean particle/text formatting은 M32 완료가 아니다. |
| M33 | body/stat/trait/experience seed and display/save owner | Strict closure complete. Source 5,378: M33-owned 5,299 implemented-verified; CFLAG/FLAG/PBAND 79 approved-excluded and received by M34. | Work/filming/training result calculations remain M37/M39/M42~M44 responsibility. The 79 CFLAG/FLAG/PBAND rows are not M33 completion; M34 closes them. |
| M34 | relationship/CFLAG/equipment/clothing owner | strict closure complete. total 2,247 implemented-verified, approved-excluded 0, blocked 0. M33 CFLAG/FLAG/PBAND 79 rows and item 211 apron behavior are closed by M34 evidence. | Event/training/mission effects remain later-owner responsibilities. mapped/source-file-review is not M34 completion. |
| M34.5 | M35 이후 완료 차단 gate | strict closure 완료. 188개 실질 hardening 단위는 implemented-verified이고, responsibilityIntegrity는 source unit이 아니라 필수 closure gate check로 분리했다. | 기능군 runtime 구현은 M34.5 책임이 아니다. M35~M52는 각자 manifest로 닫아야 한다. |
| M35 | 턴 종료/시간/hook/session cleanup | strict closure 완료. 8개 functional turn pipeline 단위는 implemented-verified이고, 기존 7개 save-field mapped row는 supporting evidence로만 남겼다. | 업무/미션/이벤트/판매/보상 내부 효과를 M35 완료로 세지 않는다. |
| M36 | 방문/시설 | strict closure 완료. 93개 source unit 중 86개 visit action group과 7개 visit place definition이 모두 implemented-verified다. 559개 coverage row는 row evidence로만 유지한다. | world/event 진행 효과는 M47 책임이고, mission/character/stat/turn/economy downstream effect는 각 owner 책임이다. |
| M37 | 업무/창관/특수 업무 | 기존 구현 이력은 있으나 strict manifest 기준 total 463, implemented 333, blocked 130. 조건/결과/roundtrip 검증을 계속 닫아야 한다. | mapped save/session/calculation row는 업무 완료가 아니다. |
| M38 | 촬영 정의/조건 | strict manifest 기준 total 6, implemented 0, blocked 6. 기존 장면 정의 체크는 전부 BLOCKED로 표기했다. | 장면명/정의만 표시하는 것은 촬영 정의 완료가 아니다. |
| M39 | 촬영 실행/판매 | 기존 구현 이력은 있으나 strict manifest 기준 total 174, implemented 135, blocked 39. 실행 결과와 턴 pipeline owner를 분리했다. | source-file-review/save/session/calculation blocked row는 M39 완료가 아니다. |
| M40 | 훈련 메뉴/session lifecycle | 기존 구현 이력은 있으나 strict manifest 기준 total 11, implemented 5, blocked 6. command availability/effect는 분리했다. | availability는 M41, effect는 M42~M44 책임이다. |
| M41 | 훈련 command availability | 기존 통과 이력은 있으나 strict manifest 기준 total 1,631, implemented 4, blocked 1,626, redesign 1. | command 효과 계산은 M42~M44 책임이다. COM_ABLE branch 조건과 불가 사유가 남아 있다. |
| M42 | 훈련 command effect 0~34 | 아직 구현 전. strict manifest 기준 total 53, blocked 53. | static profile/indexing은 효과 구현 완료가 아니다. |
| M43 | 훈련 command effect 35~69 | 아직 구현 전. strict manifest 기준 total 43, blocked 40, redesign 3. | command 70 이상과 out-of-range COMF는 M43 완료가 아니다. |
| M44 | command 70 이상/후처리 | 아직 구현 전. strict manifest 기준 total 63, blocked 40, redesign 23. | 훈련 외 event/ending 전체를 M44로 숨기지 않는다. |
| M45 | 능력 상승/휴식/공통 유지보수 | 아직 구현 전. strict manifest 기준 total 2, blocked 1, redesign 1. | 공통 시스템을 특정 기능 owner 안에 숨기면 안 된다. |
| M46 | 미션 lifecycle | 아직 구현 전. strict manifest 기준 total 1, blocked 1. | world event와 ending 판정은 M46 완료가 아니다. |
| M47 | 세계/이벤트/story | 아직 구현 전. strict manifest 기준 total 13, blocked 13. M28의 BOYFRIEND event-local 3개가 exact inbound로 추가됐고, M29/M31/M32에서 넘어온 event/world 책임도 포함된다. | 엔딩/meta 저장은 M48 책임이다. |
| M48 | 엔딩/계승/global meta | 아직 구현 전. strict manifest 기준 total 3, blocked 2, redesign 1. | 정보/설정 UI는 M49, 전체 저장 검증은 M50 책임이다. |
| M49 | 정보/도움말/설정/남은 기능 | 아직 구현 전. strict manifest 기준 total 4, blocked 3, redesign 1. M32에서 넘어온 `ZNAME.ERB` text formatting 책임이 blocked inbound다. | catch-all로 남은 기능을 숨기면 안 된다. intake 없는 기능은 원 owner로 되돌린다. |
...
상세 마일스톤의 각 체크 항목은 아래 표기를 붙인 뒤에만 완료 근거로 쓴다.

- `[HERE:Mxx]`: 이 마일스톤에서 구현/검증한다.
- `[LATER:Myy]`: 뒤 또는 다른 마일스톤 책임이다. 현재 Mxx 완료로 세지 않는다.
- `[EXCLUDED->Myy]`: 승인 제외이며 수신 Myy manifest에 책임이 있어야 한다.
- `[BLOCKED:Mxx]`: 현재 마일스톤 책임이지만 아직 못 닫았다.
- `[REDESIGN]`: owner가 섞여 있어 책임 재분리가 먼저다.
- `[VERIFY:Mxx]`: 구현은 있으나 이 마일스톤에서 검증 증거를 닫아야 한다.
- `[DOC-ONLY]`: 문서/조사 산출물이며 runtime 구현 완료가 아니다.

M32~M49는 구현 재개 전에 이 표기를 체크 항목별로 먼저 붙인다. 진행 중 이관은 금지하며, 이관이 필요하면 해당 항목을 `[REDESIGN]` 또는 `[EXCLUDED->Myy]`로 고정하고 receiver manifest를 먼저 갱신한다.
...
현재 strict 기준:
- 이 섹션의 기존 체크는 legacy coverage/gate/smoke 통과 기록이고, 현재 완료 판정은 source-unit manifest와 closure JSON이 권위다.
- M29는 구매형 listing/flow/selection 책임 101개를 implemented-verified로 닫았다.
- 비구매/사용/장비/영입/이벤트 row 105개는 approved-excluded와 receiver manifest로 추적하며 M29 완료로 세지 않는다.
...
- [x] `data/coverage/milestones/M29-closure.json` source-owner correction 적용. sourceUnitTotal 206, ownedTotal 101, implemented 101, approvedExcludedFromM29 105, ownedBlocker 0. 현재 status는 `completed`다.
...
- 범위: 즉시 사용 조건, 사용 효과, 실패/취소 경로다. 특수 item 200~214의 훈련 효과와 item 22/90/91 plus item 211 계열은 M30 완료로 계산하지 않고 수신 owner manifest에서 blocked inbound로 추적한다.
- 방식: 효과는 inventory/people/body/world/mission/settings/equipment 등 정확한 owner에만 반영한다.
- 완료 결과: strict closure 완료. source 74개 중 M30-owned 37개는 implemented-verified, 나머지 37개는 M30 approved-excluded이며 M34-closed plus M41/M42/M43/M44 수신 manifest에 blocked inbound로 명시했다.
...
- [x] 특수 item 200~214 및 item 22/90/91 plus item 211 계열 37개를 M30 책임에서 approved-excluded로 분리
- [x] M30 approved-excluded 37개를 M34-closed plus M41/M42/M43/M44 수신 manifest에 blocked inbound로 명시
...
- 완료 결과: 영입 가능한 항목 전체가 생성 결과 또는 사용자 승인 제외를 갖고 소유 blocker 0개 상태가 된다. downstream field semantics는 다른 owner가 닫더라도, recruit creation이 initial identity/body/social/time fields를 결과에 적용하는 통합 검증은 M31에서 닫는다.
...
- 2026-05-06 source-owner correction 후 M31 source 237개는 implemented-verified 153, approved-excluded 84, blocked 0로 정정했고 strict closure 완료 상태다.
- 이전에 blocked로 되돌렸던 91개 중 M31 소유 CSV/Chara initial identity/body/social/time 적용 행은 recruit creation bundle 소비 검증과 `smoke:recruit-all`로 닫았다. downstream field semantics는 M32/M33/M34/M35가 닫더라도, recruit creation이 initial fields를 생성 결과에 적용하는 검증은 M31 책임이다.
- `npm run gate:milestone-scope-closure -- M31`은 통과해야 한다.
...
- [HERE:M32] [x] M20 Chara template 109개 전체를 생성 가능한 캐릭터 원형으로 연결
- [HERE:M32] [x] 이름, 호칭, 별명, 표시 이름을 정의 데이터와 저장 상태 경계에 맞게 분리
- [HERE:M32] [x] 원형 정의와 플레이 중 인물 인스턴스를 구분
- [VERIFY:M32] [x] 삭제, 은퇴, 조수 가능, 영입 가능 상태를 저장 필드로 분리. M29 inbound TALENT 4개는 M33 approved-excluded로 분리하고 M33 manifest에 blocked inbound로 명시했다.
- [VERIFY:M32] [x] 캐릭터 정보 화면에서 identity 값 표시를 검증
- [VERIFY:M32] [x] 원형에서 인스턴스 생성 후 저장 roundtrip을 검증
- [EXCLUDED->M47/M49] [x] Chara seed 중 identity 관련 row의 coverage status 갱신. `zname`은 M49 text formatting, `c_club_girlname`/`ntr_boyfriendname_calc`는 M47 event generated-name 책임으로 receiver manifest에 blocked inbound로 명시했다.
- [VERIFY:M32] [x] `npm run build` 실행

M32 완료 근거:
- M32 strict manifest scope는 implementation queue 274개, M31 inbound transfer 20개, M29 inbound 4개를 합친 298개 row다.
- 2026-05-06 strict closure에서 M32는 source 298개 중 M32-owned 291개를 implemented-verified로 닫았다.
- approved-excluded 7개는 완료가 아니라 수신 owner 책임이다. M33 TALENT save field 4개, M47 event generated-name source-file-review 2개, M49 Korean particle/name formatting source-file-review 1개로 receiver manifest에 blocked inbound를 남겼다.
- CSTR label definition 5개는 `definitions.characterTextDefinitions`와 roster label 표시 및 `smoke:character-identity` 검증으로 `implemented-character-text-definition-label`이 되었다. mapped-consumed row는 M32 완료로 세지 않는다.
...
- [VERIFY:M33] [x] Chara seed의 `BASE`, `ABL`, `TALENT`, `EXP`, `MARK`, `PALAM` 계열을 owner별로 연결. strict manifest는 mapped-only 완료를 허용하지 않고 M33-owned row 전부를 implemented-verified로 닫는다.
- [VERIFY:M33] [x] 능력, 기초치, 소질, 경험, 각인, 파라미터 초기값을 표시 정의와 저장 수치로 분리
- [HERE:M33] [x] 신체/생식/성적 이력/오염/자원 상태를 `body` 하위 객체로 분리
- [LATER:M37/M39/M42/M43/M44] [x] 업무, 촬영, 훈련 결과가 같은 신체 필드를 공유하는지 검증. 결과 계산 자체는 업무/촬영/훈련 effect owner가 닫고 M33은 seed/display/save owner만 닫는다.
- [VERIFY:M33] [x] 값 범위, 레벨 표시, 증가/감소 계산 기준을 정의 데이터와 연결
- [VERIFY:M33] [x] 인물 정보 화면과 저장 roundtrip을 검증
- [VERIFY:M33] [x] M20/M24 coverage의 body 관련 status 갱신. mapped/transferred 기반 구판정은 제거하고 strict manifest 기준 source 5,378, M33-owned 5,299, approved-excluded 79로 닫았다.
- [VERIFY:M33] [x] `npm run build` 실행

M33 완료 근거:
- M33 source scope는 M27 implementation queue의 `unit:M33:body-stat` 5,283행, M33 본문 필수 범위인 `Palam.csv` training param 정의 17행, M31 inbound 74행, M32 inbound 4행을 합쳐 총 5,378행이다.
- 2026-05-06 strict closure에서 M33-owned 5,299행은 전부 implemented-verified로 닫았다. mapped row와 transferredOut row는 M33 완료 근거로 허용하지 않는다.
...
- M33 mixed CFLAG/FLAG/PBAND 79 rows are not M33 completion; they were approved-excluded from M33 and are now implemented-verified by M34.
...
- `data/coverage/body-stat-coverage.json`, `data/coverage/audits/M33-gap-audit.json`, `data/coverage/manifests/M33-source-units.json`, `data/coverage/milestones/M33-closure.json`을 갱신했다. closure는 sourceUnitTotal 5,378, ownedTotal 5,299, implemented 5,299, approvedExcludedFromM33 79, mapped 0, transferredOut 0, blocker/missing/unapproved 0이다.
...
- M34 strict closure is complete: total 2,247, implemented-verified 2,247, blocked 0. Mapped rows are no longer counted as completion.
- [HERE:M34] [x] 관계값과 방향성 관계를 `social`에 연결
- [HERE:M34] [x] 장비, 의복, 착용 상태, 해금 상태를 `equipment`와 `inventory/shop/world` 경계에 맞게 분리
- [VERIFY:M34] [x] CFLAG 중 people/body/equipment/social/work/mission/settings/features owner를 구분
- [BLOCKED:M34] [x] 의미 불명 CFLAG는 완료 처리하지 않고 blocker로 남김
- [VERIFY:M34] [x] 관계/장비/의복 변화가 저장 roundtrip에 남는지 검증
- [VERIFY:M34] [x] 원본명 `CFLAG`를 앱 모델명으로 직접 사용하지 않도록 검색
- [VERIFY:M34] [x] M34 strict closure now has total 2,247, implemented-verified 2,247, blocked 0. Mapped rows are not completion evidence.
- [VERIFY:M34] [x] `npm run build` 실행

M34 완료 근거:
- M34 strict manifest scope is now total 2,247 after duplicate placeholder blockers were removed and item 211/inbound responsibilities were implemented.
...
- [HERE:M34.5] [x] `gate:source-evidence`의 완료성 상태 차단 목록을 확장한다. `implemented`, `used`, `template`, `listing`, `display-only`, `calculation-only`, `mapped`, `non-save`, `session-field`, `calculation-internal`, `script-scratch`, `approved-excluded`는 auxiliary evidence만으로 완료될 수 없다.
- [HERE:M34.5] [x] `gate:source-evidence`가 `features.json`, `definitions.json`, `blockers.json`, `save-mapping.json`, `session-mapping.json`, `approved-exclusions.json`, `implementation-queue.json`, 마일스톤별 `*-coverage.json`을 모두 검사하게 한다.
- [HERE:M34.5] [x] auxiliary source row 169개를 primary source evidence에 연결하거나 완료성 상태에서 빼고 blocker/owner review로 되돌린다.
- [HERE:M34.5] [x] `source-file-review` row는 파일명 단위 계약만으로 완료 처리하지 못하게 한다. 완료하려면 라벨, CSV 행, family/index, read/write 방향 중 하나 이상의 primary row로 분해해야 한다.
- [HERE:M34.5] [x] `coverage-gate-registry.json`을 만든다. M35~M52 각 마일스톤의 coverage file, gate command, smoke command, closure file, gap audit file, 필수 row field를 기록한다.
- [HERE:M34.5] [x] `gate:coverage-hardening`을 만든다. registry에 등록된 모든 script가 `package.json`에 있고, 각 gate가 존재하며, 완료성 row의 `sourceEvidenceId`, `runtimeConsumerId`, `verificationId` 누락을 실패시켜야 한다.
- [HERE:M34.5] [x] M35~M49 전용 coverage/gate/smoke skeleton을 추가한다. skeleton은 아직 구현 row가 없으면 성공하지 말고, 해당 owner row가 미구현임을 실패 또는 blocker로 드러내야 한다.
- [HERE:M34.5] [x] `audit:final-gap`, `gate:final-gap-audit`, `gate:final-content-zero-gap` skeleton을 추가한다.
- [HERE:M34.5] [x] `coverage:view-text`, `gate:view-and-text-coverage` skeleton을 추가한다.
...
- [HERE:M34.5] [x] M34.5 closure의 `responsibilityIntegrity`는 필수 gate check로 통과시켰다. 단, 자기참조 완료를 막기 위해 source-unit 완료 수에는 넣지 않고, strict manifest는 188개 실질 hardening 단위 implemented-verified 188, blocked 0으로 닫았다.
...
- [VERIFY:M34.5] [x] `npm run gate:pre-implementation-audit` 실행
- [VERIFY:M34.5] [x] `npm run gate:implementation-queue` 실행
- [VERIFY:M34.5] [x] `npm run build` 실행
- [VERIFY:M34.5] [x] `npm run test --if-present` 실행
...
- [HERE:M35] [x] 원본 턴 종료 흐름의 day/week/month/year 진행 규칙을 구현
- [HERE:M35] [x] 전반/후반 또는 phase 전환 규칙을 저장 상태와 session 폐기로 분리
- [HERE:M35] [x] 턴 종료 전 hook, 턴 종료 후 hook, 월말 hook, 새 주 hook을 정의
- [LATER:M36/M37/M41/M46/M47] [x] 자동 구매, 자동 아이템 사용, 업무 결과, 미션 기한, 이벤트 발생 순서를 연결. 각 내부 효과는 해당 기능 owner가 닫고 M35는 호출 순서와 lifecycle만 닫는다.
- [LATER:M39/M46/M47] [x] 월말 비용, 판매, 보상, 패널티가 `economy/run/world` owner에 반영되는지 검증. M35는 turn pipeline owner이며 결과 계산 owner가 아니다.
- [HERE:M35] [x] 턴 종료 중 session 선택값이 남지 않는지 검증
- [HERE:M35] [x] 턴 종료 후 저장 roundtrip을 검증
- [HERE:M35] [x] M19/M24/M25 coverage의 turn-end 관련 status 갱신
- [VERIFY:M35] [x] `npm run build` 실행

M35 완료 근거:
- M35 strict manifest는 8개 functional turn pipeline 단위다. 기존 implementation queue 4행과 M29/M31 inbound transfer 3행의 save-field row는 supporting evidence로만 유지한다.
...
- `data/coverage/turn-pipeline-coverage.json`, `data/coverage/audits/M35-gap-audit.json`, `data/coverage/manifests/M35-source-units.json`, `data/coverage/milestones/M35-closure.json`을 strict 기준으로 갱신했다. ownedTotal 8, implemented 8, mapped 0, blocker/missing/unapproved 0.
...
- [HERE:M36] [x] 방문 장소 전체를 장소 정의 source unit 7개로 분류하고, runtime consumer와 smoke 검증으로 implemented-verified 처리
- [HERE:M36] [x] 장소별 행동 전체를 구현 또는 사용자 승인 제외로 닫고, 소유 blocker 0개 확인
- [HERE:M36] [x] 장소 해금 조건과 현재 선택 session을 분리
- [VERIFY:M36] [x] 비용 부족, 조건 미충족, 중복 실행, 취소를 검증
- [LATER:M47] [x] 시설 해금과 세계 진행 상태를 `world` 또는 `featureState`에 연결. world/event 진행 효과는 M47에서 닫는다.
- [LATER:M32/M33/M34/M35] [x] 방문 행동 결과가 인물, 신체, 돈, 시간 진행에 반영되는 경우 owner를 명시. M36은 방문 action owner이며 downstream result owner를 완료로 세지 않는다.
- [VERIFY:M36] [x] 방문 후 저장 roundtrip을 검증
- [VERIFY:M36] [x] M19/M21/M24/M25 coverage의 방문 관련 status 갱신
- [VERIFY:M36] [x] `npm run build` 실행

M36 완료 근거:
- M36 strict owned scope는 93개 source unit이다. visit place definition 7개는 `visitPlaceDefinitions -> createVisitSession/buildVisitView`로 소비하고, visit feature 552행은 source file + source label 기준 86개 `visitActionDefinitions` action으로 묶어 소비한다.
...
- `data/coverage/visit-facility-coverage.json`, `data/coverage/audits/M36-gap-audit.json`, `data/coverage/manifests/M36-source-units.json`, `data/coverage/milestones/M36-closure.json`을 strict 기준으로 갱신했다. source units 93, implemented-verified 93, mapped 0, blocked 0, blocker/missing/unapproved 0. 559 coverage row는 row evidence로만 유지한다.
...
- [BLOCKED:M37] [ ] 업무 결과가 돈, 인물, 신체, 경험, 업무 이력, 시간 진행에 반영되는지 검증. 현재 strict manifest 기준 save/session/calculation/source-address 130개가 결과 구현 증거 없이 남아 있다.
...
- [BLOCKED:M38] [x] 촬영 장면 전체를 장면 정의 또는 사용자 승인 제외로 닫고, 소유 blocker 0개 확인
- [BLOCKED:M38] [x] 촬영 대상 후보 조건과 장면 후보 조건을 view 계산으로 구현
- [BLOCKED:M38] [x] 장면별 표시 이름, 설명, 요구 상태, 예상 결과를 정의 데이터로 연결
- [BLOCKED:M38] [x] 장면 선택, 대상 선택, 조건 실패, 취소를 검증
- [LATER:M39] [x] 촬영 중간 계산값은 session/calculation에만 둠. 실행 중 계산 lifecycle은 M39에서 닫는다.
- [BLOCKED:M38] [x] 장면 정의가 CSV/ERB 근거를 갖는지 확인
...
- [HERE:M39] [x] 촬영량, 수익, 팬, 점수, 경험, 신체 피로 계산을 구현
- [VERIFY:M39] [x] 촬영 결과가 `economy`, `people`, `body`, `work` owner에만 반영되는지 검증
- [HERE:M39] [x] 촬영 이력과 출시/판매 상태를 저장 상태에 연결
- [VERIFY:M39] [x] 촬영 실패, 조건 미충족, 장면 누락, 대상 누락, 취소를 검증
- [LATER:M35] [x] 촬영 완료 후 턴 종료와 session 폐기를 검증. 턴 pipeline 자체는 M35가 닫는다.
- [VERIFY:M39] [x] 촬영 후 저장 roundtrip을 검증
...
- [HERE:M40] [x] 훈련 대상, 실행자, 조수, command 선택 상태를 session owner에 연결
- [LATER:M41] [x] 훈련 command 후보 view를 전체 command 기준으로 계산. availability 조건과 불가 사유는 M41 책임이다.
- [VERIFY:M40] [x] 대상/실행자/조수 누락, 불가 조건, 취소를 검증
- [HERE:M40] [x] 훈련 장비/임시 모드/선택값은 session에만 둠
- [VERIFY:M40] [x] 훈련 화면 진입과 복귀가 저장 상태를 직접 바꾸지 않도록 검증
- [BLOCKED:M40] [x] 훈련 session이 완료/취소/턴 종료 시 폐기되는지 검증
...
- [BLOCKED:M41] [x] `Train.csv` 105개 command별 가능 조건을 구현 또는 사용자 승인 제외로 닫고, 소유 blocker 0개 확인
- [BLOCKED:M41] [x] command별 대상, 실행자, 조수 역할 조건을 구현
- [BLOCKED:M41] [x] command별 장비, 상태, 자원, 장소, 이벤트 조건을 구현
- [BLOCKED:M41] [x] 불가 사유를 view 계산 결과로 표시
- [VERIFY:M41] [x] availability 계산이 저장 상태를 바꾸지 않는지 검증
- [BLOCKED:M41] [x] command별 가능/불가 smoke 또는 table 검증을 추가
...
- [BLOCKED:M42] [ ] command 0~34 원본 단위 매니페스트를 작성하고 각 command row와 연결 `COMF*.ERB` source를 적음
- [HERE:M42] [ ] command 0~34의 source 계산, 파라미터 증감, 체력/기력 감소를 구현
- [EXCLUDED->M30/M41] [ ] 특수 item 소비 조건이 있으면 item owner와 effect owner를 매니페스트에서 분리하고 완료/차단 상태를 기록
- [HERE:M42] [ ] command별 결과 owner를 `people`, `body`, `social`, `inventory`, `economy`, `run` 중 하나로 확정
- [VERIFY:M42] [ ] command별 성공, 불가, 취소, 결과 적용, session 폐기를 검증
- [VERIFY:M42] [ ] 원본 계산 중간값을 저장 payload에 넣지 않도록 검증
- [BLOCKED:M42] [ ] command 0~34의 source evidence와 consumer evidence를 연결
- [BLOCKED:M42] [ ] 관련 blocker 0개 확인. 해소 불가 항목은 사용자 승인 제외 근거가 있을 때만 완료 처리
- [BLOCKED:M42] [ ] M20/M24/M25 coverage의 command 0~34 status 갱신
- [VERIFY:M42] [ ] `npm run build` 실행
...
- [BLOCKED:M43] [ ] command 35~69 원본 단위 매니페스트를 작성하고 각 command row와 연결 `COMF*.ERB` source를 적음
- [HERE:M43] [ ] command 35~69의 source 계산, 파라미터 증감, 체력/기력 감소를 구현
- [EXCLUDED->M30/M41] [ ] 특수 item 소비 조건이 있으면 item owner와 effect owner를 매니페스트에서 분리하고 완료/차단 상태를 기록
- [HERE:M43] [ ] command별 결과 owner를 `people`, `body`, `social`, `inventory`, `economy`, `run` 중 하나로 확정
- [VERIFY:M43] [ ] command별 성공, 불가, 취소, 결과 적용, session 폐기를 검증
- [VERIFY:M43] [ ] 원본 계산 중간값을 저장 payload에 넣지 않도록 검증
- [BLOCKED:M43] [ ] command 35~69의 source evidence와 consumer evidence를 연결
- [BLOCKED:M43] [ ] 관련 blocker 0개 확인. 해소 불가 항목은 사용자 승인 제외 근거가 있을 때만 완료 처리
- [BLOCKED:M43] [ ] M20/M24/M25 coverage의 command 35~69 status 갱신
- [VERIFY:M43] [ ] `npm run build` 실행
...
- [REDESIGN] [ ] command 70 이상과 번호 범위 밖 훈련 효과 source의 원본 단위 매니페스트를 작성
- [HERE:M44] [ ] command 70 이상 전체의 source 계산, 파라미터 증감, 체력/기력 감소를 구현
- [HERE:M44] [ ] `COMF137.ERB`처럼 특수 item/후처리에서 소비되는 훈련 효과 source를 구현 또는 차단 상태로 닫음
- [EXCLUDED->M34/M47] [ ] 훈련 후처리, 이벤트, 장비 변화, 자원 변화가 올바른 owner에 반영되는지 검증
- [VERIFY:M44] [ ] command별 성공, 불가, 취소, 결과 적용, session 폐기를 검증
- [VERIFY:M44] [ ] 원본 계산 중간값을 저장 payload에 넣지 않도록 검증
- [BLOCKED:M44] [ ] 전체 command/effect source coverage에 미구현 row가 남지 않는지 확인
- [VERIFY:M44] [ ] 원본명 `COMF`, `TFLAG`, `SOURCE`, `TEQUIP`, `LOSEBASE` 직접 사용 검색 통과
- [BLOCKED:M44] [ ] M20/M24/M25 coverage의 command 70 이상 및 후처리 status 갱신
- [VERIFY:M44] [ ] `npm run build` 실행
...
- [BLOCKED:M45] [ ] 능력 상승 메뉴와 조건을 구현 또는 사용자 승인 제외로 닫고, 소유 blocker 0개 확인
- [HERE:M45] [ ] 휴식, 회복, 자동 아이템 사용, 공통 유지보수 흐름을 구현
- [REDESIGN] [ ] 공통 시스템 feature가 특정 기능 owner에 잘못 섞이지 않도록 분리
- [VERIFY:M45] [ ] 성공, 조건 미충족, 취소, 턴 종료 연결을 검증
- [LATER:M32/M33/M34/M35] [ ] 결과가 `people`, `body`, `inventory`, `run`, `economy` owner에만 반영되는지 검증. 결과 owner별 저장 의미는 해당 owner manifest와 대조한다.
- [VERIFY:M45] [ ] 저장 roundtrip을 검증
- [BLOCKED:M45] [ ] M19/M20/M24/M25 coverage의 common-system 관련 status 갱신
- [VERIFY:M45] [ ] `npm run build` 실행
...
- [BLOCKED:M46] [ ] 미션 전체를 정의, 진행 상태, 보상, 실패 조건으로 분리
- [HERE:M46] [ ] 미션 수락, 진행, 보고, 완료, 실패, 만료를 구현
- [LATER:M35] [ ] 미션 기한과 턴 종료 처리를 연결. 턴 hook 실행 순서는 M35가 닫는다.
- [LATER:M32/M33/M34/M47] [ ] 미션 보상과 패널티가 `economy`, `world`, `people`, `body`, `featureState`에 반영되는지 검증. 결과 저장 owner는 각 owner manifest와 대조한다.
- [VERIFY:M46] [ ] 미션 목록 선택 session이 저장 payload에 남지 않는지 검증
- [VERIFY:M46] [ ] 미션 후 저장 roundtrip을 검증
- [BLOCKED:M46] [ ] M19/M20/M24/M25 coverage의 미션 관련 status 갱신
- [VERIFY:M46] [ ] `npm run build` 실행
...
- [BLOCKED:M47] [ ] 이벤트 발생 조건, 1회성/반복성, 결과 상태 변화를 구현 또는 사용자 승인 제외로 닫고, 소유 blocker 0개 확인
- [HERE:M47] [ ] 세계/장소/스토리 진행 플래그를 `world`에 연결
- [HERE:M47] [ ] 이벤트 표시 화면과 상태 변경을 분리
- [LATER:M35/M36/M39/M41/M46] [ ] 턴 종료, 방문, 미션, 훈련, 촬영에서 발생하는 이벤트 hook을 연결. 호출 위치는 각 기능 owner가 갖고 M47은 이벤트 조건/효과를 닫는다.
- [VERIFY:M47] [ ] 이벤트 결과가 저장 상태와 session에 섞이지 않도록 검증
- [VERIFY:M47] [ ] 이벤트 후 저장 roundtrip을 검증
- [BLOCKED:M47] [ ] M19/M23/M24/M25 coverage의 event-world 관련 status 갱신
- [VERIFY:M47] [ ] `npm run build` 실행
...
- [BLOCKED:M48] [ ] 엔딩 조건 전체를 조건 id와 결과 화면으로 분류
- [HERE:M48] [ ] 엔딩 조건 계산을 view/calculation으로 구현
- [HERE:M48] [ ] 엔딩 결과, 계승 새 게임, 게임 종료 흐름을 구현
- [HERE:M48] [ ] 회차 밖 업적, 클리어 보너스, 전역 해금을 `meta`에 연결
- [REDESIGN] [ ] LOADGLOBAL/SAVEGLOBAL 성격의 상태와 회차 save 상태를 분리 검증
- [VERIFY:M48] [ ] 엔딩 후 저장/로드 또는 새 게임 흐름을 검증
- [BLOCKED:M48] [ ] M19/M24/M25 coverage의 ending-event-meta 관련 status 갱신
- [VERIFY:M48] [ ] `npm run build` 실행
...
- [BLOCKED:M49] [ ] 정보, 도움말, 업적, 설정, 디버그 메뉴를 구현 또는 사용자 승인 제외로 분류
- [REDESIGN] [ ] `M49-intake` 표를 만들고 source id, 원래 owner 후보, M49로 온 이유, 승인 제외 여부를 기록
- [REDESIGN] [ ] intake 없는 잔여 기능은 해당 owner 마일스톤으로 되돌리거나 `scope-redesign-required`로 차단
- [HERE:M49] [ ] PRINT/HTML/message/help/status text를 view/text coverage로 분류하고 실제 소비 화면을 연결
- [LATER:M48] [ ] 설정 상태를 `settings`와 `meta` owner로 분리. 전역 meta 저장은 M48/M50와 경계를 확인한다.
- [VERIFY:M49] [ ] 정보 화면이 정의 데이터와 저장 상태를 읽기 전용으로 표시하는지 검증
- [EXCLUDED->M49] [ ] 디버그 기능은 개발 전용 또는 사용자 승인 제외로 분류하고, 미승인 blocker가 있으면 완료하지 않음
- [VERIFY:M49] [ ] M28~M49 전체 기능군의 gap audit을 취합
- [BLOCKED:M49] [ ] 기능군별 미구현 feature, 미소비 definition, 미정 save/session row가 남지 않는지 확인
- [VERIFY:M49] [ ] `npm run build` 실행
- [VERIFY:M49] [ ] 테스트 도구가 있으면 `npm run test --if-present` 실행
