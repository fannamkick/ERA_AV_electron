...
| M37 | 463 | 324 | 139 | 0 | no | 286 work feature rows, 8 work definitions, 21 work result write-effect rows, 1 LUNCH_STALL ABL:74 reward read row, and 8 EVENT_WORK_MESSAGE_SP message branch read rows are implemented-verified. Remaining save/session/calculation/source-address rows remain blocked. |
...
| M37 | old completed, implemented 286, mapped 175 | blocked | 예 | 업무 실행, 업무 정의, save-field owner, session/calculation owner가 섞임. strict 재판정 후 total 463, implemented-verified 324, blocked 139 | M37이 닫을 업무 실행 단위와 save/session/calculation 결과 단위를 분리하고, blocked 139을 구현 또는 scope redesign |
...
| M37 | 업무/창관/특수 업무 실행 80개, 업무 정의 8개, 업무 결과 write-effect 21개와 LUNCH_STALL ABL:74 보상 read 1개, EVENT_WORK_MESSAGE_SP message branch read 8개는 구현 검증됐다. strict 기준 implemented-verified 324. | 남은 save/session/calculation/source-address 139개는 M37 완료가 아니다. | M37 closure는 blocked 139이다. 업무 결과의 원본 save-field 효과를 구현하거나 명시적으로 scope redesign 해야 한다. |
