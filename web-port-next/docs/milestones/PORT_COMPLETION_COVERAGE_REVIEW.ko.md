...
- M34.5~M41은 구현 성격이지만, 일부 closure가 `mapped`와 `transferredOut`을 완료 totals에 포함한다. 이 경우 "책임을 끝냈다"가 아니라 "분류하거나 넘겼다"일 수 있다. M28~M34은 strict closure로 보정됐다.
...
현재 그대로 M42 이후를 진행하면 놓칠 수 있는 부분이 있다. 2026-05-02 재판정 이후 M28~M34은 strict closure로 보정됐지만, M34.5~M41은 M42 전에 원본 단위 매니페스트 기준으로 보강하거나 blocked/scope-redesign-required로 정정해야 한다.
...
- M34.5~M41은 `M28_M41_DONE_NOT_DONE_LEDGER.ko.md`의 2026-05-02 원본 단위 매니페스트 기준 재판정 표를 따른다. M28~M34은 strict closure 완료 상태다.
...
| 우선순위 | 대상 | 해야 할 일 |
| --- | --- | --- |
| 1 | M34.5~M41 전체 | 기존 `completed`를 유지하지 않는다. 각 마일스톤은 원본 단위 매니페스트를 만들고 단위별 `implemented-verified`/`approved-excluded`/`blocked`/`scope-redesign-required`로 닫는다. |
...
| 6 | M42 | M34.5~M41 보강/정정 전에는 시작하지 않는다. |
