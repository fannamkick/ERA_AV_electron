...
| M | 원래 맡은 일 | 실제 처리 | 완료로 세면 안 되는 것 / 다음 책임 |
| --- | --- | --- | --- |
| M21 | 원본 근거 장부 확정 | source evidence schema, source manifest, feature/definition row의 sourceEvidence 연결, auxiliary evidence 분리 gate를 만들었다. | 원본 동작 구현은 하지 않았다. 문서/보조 CSV만으로 runtime 구현 완료를 세면 안 된다. |
| M22 | 전수표 교차 대조 gate | feature/definition/save/session/blocker 교차 대조 규칙, approved exclusion 형식, orphan/role-only/source 누락 실패 조건을 만들었다. | 기능군 구현은 하지 않았다. gate 통과는 구현 완료가 아니라 장부 무결성 기준이다. |
| M23 | ERB 기반 정의 후보 보강 | ERB-only 정의 160행과 component label 302개를 산출하고 definition coverage에 병합했다. | ERB-only 컨텐츠를 runtime에서 소비하는 구현은 M28~M49 owner가 닫는다. |
| M24 | persistent 후보 save owner 매핑 | source row 1,215개와 persistent 후보 1,016개를 save field/non-save/session 등으로 분류했다. 31개는 M25 session-state로 넘겼다. | save owner 매핑은 기능 구현이 아니다. 해당 field를 실제로 읽고 쓰는 기능은 M28 이후 owner가 닫는다. |
| M25 | session/calculation 후보 lifecycle 매핑 | source row 365개와 runtime session 후보 234개를 session-field/calculation-internal 등으로 분류했다. `ITEMSALES`, `EX`, `GOTJUEL` 등 M24 이관분을 포함했다. | session mapping은 session 사용 기능 구현이 아니다. 생성/소비/폐기는 각 기능 owner가 검증한다. |
| M26 | 구현 전 누락 감사 | source-file-review 14개, implementation review 14,546행, owner/closureRule/verificationCommand를 만들고 unresolved gap 0으로 정리했다. | 감사는 구현이 아니다. 발견된 row는 M27 queue와 이후 owner가 실제로 닫는다. |
| M27 | 구현 큐와 blocker 동결 | M28~M49/M50~M51 queue unit 36개, queued review row 14,546개, blocker 59개, approved exclusion request 59개를 동결했다. | 큐 항목 구현은 하지 않았다. M27 owner로 남은 source-file-review 2개는 M51 감사 owner로 넘겼다. |
