...
Next order:
1. M34.5 closure correction.
2. M35~M41 strict manifest closure correction.
3. M42 command effect 0~34 implementation only after M34.5~M41 are closed or explicitly blocked/scope-redesign-required.
...
Required M32 flow:
1. Re-read `data/coverage/manifests/M32-source-units.json`, `data/coverage/character-identity-coverage.json`, and `data/coverage/audits/M32-gap-audit.json`.
...
4. Run `npm run coverage:character-identity`, `npm run gate:character-identity`, `npm run gate:milestone-scope-closure -- M32`, `npm run smoke:recruit-all`, and `npm run build`.
...
M35-M41 source-unit manifest pass 1 is complete. M28~M34 have since been closed under the strict source-unit rule; M34.5-M41 still need blocked/scope-redesign-required closure before M42 implementation resumes.
...
3. M34.5~M41 전체에서 `completedAllowedNow: true` 또는 정당한 blocked/scope-redesign-required closure가 확정된 뒤 M42 command effect 구현을 재개한다. M28~M34은 strict closure 완료 상태다.

Current strict status: M34 is complete at 2,247/2,247 implemented-verified. M34.5~M41 still follow their own source-unit manifests and must be closed or explicitly blocked/scope-redesign-required before M42 resumes.

## M34.5~M41 완료 선언 재정렬 후 M42. 훈련 command 효과 0~34 완성

현재 즉시 목표는 M34.5~M41 완료 선언을 원본 단위 매니페스트 기준으로 보강하거나 blocked/scope-redesign-required로 정정하는 것이다. M28~M34은 strict closure로 완료됐다. 그 다음에야 M42 command 0~34의 원본 효과 계산과 결과 반영을 실제 runtime behavior, source evidence, consumer evidence, 검증으로 닫는다.

M28~M34 source-unit manifests are strict-closed. Next work starts at M34.5 closure correction, then M35~M41.
...
- `../milestones/M28_M41_DONE_NOT_DONE_LEDGER.ko.md`의 2026-05-02 재판정 표를 기준으로 M34.5~M41을 순서대로 보강한다. M28~M34은 완료 상태다.
- M29~M31은 strict closure 완료 상태다. 다음 작업은 `../../data/coverage/manifests/M32-source-units.json` 기준으로 M32를 정리한다.
- M34.5~M41의 `원본 단위 매니페스트`와 `responsibilityIntegrity` 보강 또는 blocked/scope-redesign-required closure 정정이 끝나기 전에는 M42 구현을 시작하지 않는다.
