...
- M34.5~M41 must be closed or explicitly redesigned before M42 implementation resumes. M28~M34 are now strict-closed.
- Current aggregate: total units 11,226; implemented-verified 8,035; approved-excluded 273; blocked 2,819; scope-redesign-required 99; completedAllowedNow true 4 / false 22.
...
| `milestones/PORT_RESPONSIBILITY_MAP.ko.md` | 프로젝트/페이즈/마일스톤 3층 책임 지도 | 검증 로그, 완료 이력 |
| `milestones/RESPONSIBILITY_SEPARATION_RULES.ko.md` | completed/blocked/scope-redesign-required 판정 기준과 의심 신호 | 개별 구현 이력 |
...
- 현재 마일스톤의 상세 체크리스트는 `milestones/README.ko.md`에서 해당 phase 문서를 찾아 그 section만 좁게 조회한다. 책임 범위는 `milestones/PORT_RESPONSIBILITY_MAP.ko.md`와 해당 phase 문서의 `페이즈 책임`을 우선한다. 완료 판정 전에는 원본 단위 매니페스트를 만들고 `milestones/RESPONSIBILITY_SEPARATION_RULES.ko.md`를 확인한다.
