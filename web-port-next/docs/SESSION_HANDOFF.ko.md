...
## 2026-05-06 handoff after M28/M29/M31 source-owner corrections

The current goal remains the complete web port of the original game. A milestone is not closed by moving rows to another owner name; it is closed only when its own gameplay responsibility is implemented and verified, explicitly approved-excluded with receiver responsibility, blocked, or scope-redesigned.

Current state:
- M28 is strict-closed. Its 3 BOYFRIEND event-local rows now have exact blocked inbound units in `data/coverage/manifests/M47-source-units.json`.
- M29 is strict-closed after source-owner correction: source 206, M29-owned implemented-verified 101, approved-excluded 105, blocked 0, `completedAllowedNow: true`.
- M31 is strict-closed after source-owner correction: source 237, M31-owned implemented-verified 153, approved-excluded 84, blocked 0, `completedAllowedNow: true`.
- M47 now has total 13 blocked units because it receives the exact M28 event-local rows.
- Aggregate M28~M52 state: total 11,250; implemented-verified 8,888; approved-excluded 315; blocked 2,016; scope-redesign-required 31; completedAllowedNow true 10 / false 16.

OpenRouter worker MCP policy:
- Use OpenRouter worker MCP actively for bounded parallel work: source/manifest cross-checks, blocked row classification, gate failure log analysis, and patch proposals.
- Do not treat worker output as completion authority. Codex must verify worker findings against original rows, coverage/gap/closure artifacts, and gate/smoke/build results before applying or committing.
- Keep worker scope narrow with explicit readonly/editable globs and allowed commands. Do not pass `.env.local`, secrets, or unrelated dirty files.
...
Next:
- Start M37 closure correction, then M38~M41. Do not resume M42 until M37~M41 are closed or explicitly blocked/scope-redesign-required under the source-unit manifest rule.
...
- M29 is strict-closed after source-owner correction.
- M30 is strictly closed.
- M31 is strict-closed after source-owner correction.
- M32 is strictly closed.
- M34, M34.5, M35, and M36 are strictly closed.
- The immediate next work is M37 closure correction, then M38~M41.
...
M31 is strict-closed under the source-unit manifest rule after source-owner correction.

- Manifest: `data/coverage/manifests/M31-source-units.json`
- Closure: `data/coverage/milestones/M31-closure.json`
- M31 summary: total 237, implemented-verified 153, approved-excluded 84, blocked 0, scope-redesign-required 0, `completedAllowedNow: true`.
...
- M29 summary: total 206, implemented-verified 101, approved-excluded 105, blocked 0, scope-redesign-required 0, `completedAllowedNow: true`.
...
- M30 was closed later; M31 has since been source-owner corrected and strict-closed.
...
- Aggregate totals: total units 11,250; implemented-verified 8,888; blocked 2,016; scope-redesign-required 31; approved-excluded 315.
...
- M37: `data/coverage/manifests/M37-source-units.json`, total 463, implemented-verified 333, blocked 130.
...
M28 is now closed under strict completion. The next worker must not resume M42 before closing or explicitly redesigning M37-M41 blocked/scope-redesign-required units.
...
- M31: `data/coverage/manifests/M31-source-units.json`, total 237, implemented-verified 153, approved-excluded 84, blocked 0, completedAllowedNow true.
...
Current criteria: M28~M36 are strict-closed after M28/M29/M31 correction. Do not mistake M37~M41 blocked/scope-redesign-required units for completion. M42 is still not resumed.

New sessions should continue with M37~M41 source-unit manifest closure correction. M42 command effect 0~34 resumes only after that.
...
- OpenRouter worker MCP는 bounded parallel work에 적극 사용한다. 단, worker 결과는 완료 판정 권위가 아니며 Codex 본체가 원본/coverage/gate로 재검토한다.
...
- M29는 source-owner correction 후 strict-closed다. `unit:M29:shop-purchase` 206행 중 implemented-verified 101, approved-excluded 105, blocked 0이다. 즉시 사용 item 효과는 M30이지만 item 30/31/38/39/40/41/42/43/52의 listing/ITEMSALES/구매 선택은 M29 책임으로 닫았다.
...
- M31은 source-owner correction 후 strict-closed다. source 237행 중 implemented-verified 153, approved-excluded 84, blocked 0이다. downstream field semantics는 M32/M33/M34/M35가 닫더라도 recruit creation result integration은 M31 책임으로 닫았다.
...
- M37은 strict 기준으로 아직 닫히지 않았다. 업무/창관/특수 업무 dispatch와 80개 업무 정의 실행, 일부 업무 결과 write-effect는 검증됐지만, strict manifest 기준 total 463 중 implemented-verified 333, blocked 130이다.
- M37 blocked 130은 남은 save/session/calculation/source-address row다. `gate:work-coverage`의 old mapped 175는 완료 근거가 아니며, `gate:milestone-scope-closure -- M37`는 통과하면 안 된다.
- 2026-05-07 M37 generator correction: `tools/build_work_coverage.mjs` now writes `data/coverage/milestones/M37-closure.json` from the strict source-unit manifest, so rerunning `npm run coverage:work` cannot recreate the old false `completed` closure. `gate:work-coverage` also checks that closure status follows `M37-source-units.json`.
...
New sessions should continue with M37~M41 source-unit closure work. M28/M29/M31 correction is complete. M38~M41 closure files are `blocked`, not completed. M42 command effect 0~34 resumes only after these blockers are actually closed or explicitly left blocked/scope-redesign-required.
- M28~M36 manifest 기준 현재 상태: M28~M36은 strict-closed다. M28의 M47 inbound 3개 정확 수신 correction도 완료했다. M29/M30/M32/M33 excluded row는 수신 manifest에 inbound로 명시되어 후속 owner 완료를 막는다.
...
- M37 work coverage/gate/smoke는 통과하지만 strict closure는 blocked 130 때문에 미완료
- 2026-05-07 M37 추가 정정: direct work read-condition 9개(CFLAG:28, EXP:11, EXP:74, EXP:75, FLAG:51, ITEM:19, PBAND:1, TALENT:133, TALENT:510)를 save-field write로 위장하지 않고 `calculateWorkResult`/`computeVisibleWorkIds` 소비와 `smoke:work-all` 단언으로 닫았다.
...
24. M29 아이템 상점과 구매는 source-owner correction 후 strict-closed다. total 206, implemented-verified 101, approved-excluded 105, blocked 0이다.
...
26. M31 영입 listing과 인물 생성은 source-owner correction 후 strict-closed다. total 237, implemented-verified 153, approved-excluded 84, blocked 0이다.
...
34. M37 업무/창관/특수 업무는 strict 재판정 결과 아직 완료가 아니다. `coverage:work`, `gate:work-coverage`, `smoke:work-all`은 업무 dispatch/정의 실행 및 일부 업무 결과 write-effect 증거지만, `gate:milestone-scope-closure -- M37`는 strict 기준에서 실패해야 한다. 현재 M37은 total 463, implemented-verified 333, blocked 130이다.
   - 최신 M37 정정: direct work read-condition 9개를 실제 runtime branch/availability/reward/target-selection 소비로 구현했다. 남은 130개는 아직 M37 안에서 구현 또는 scope redesign이 필요하다.
...
39. 다음 작업은 M37~M41 실제 기능 closure, 그 다음 M42 훈련 command 효과 0~34 구현이다. command 0~34의 source 계산, 결과 owner, 성공/불가/취소/session cleanup을 원본 기준으로 닫는다.
...
New sessions should continue with M37~M41 source-unit manifest closure correction. M28~M36 are strict-closed after M28/M29/M31 correction. M42 command effect 0~34 resumes only after M37~M41 are closed or explicitly blocked/scope-redesign-required.
- Additional source-owner recheck across M28~M41: M29 and M31 were the confirmed reclaim corrections and are now strict-closed. M28 was not a source-owner reclaim but exact M47 receiver inbound correction is now recorded. M30/M32/M33 exclusions are conditionally valid by source-owner role, while M37~M41 are already blocked and must be closed by actual gameplay implementation or scope redesign.
- M42는 완료가 아니다.
