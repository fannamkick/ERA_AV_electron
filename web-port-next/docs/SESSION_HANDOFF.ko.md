...
새 세션은 아래 요약만 읽고 M39 촬영 실행/결과/판매 완성부터 진행한다. M38 촬영 정의와 장면 조건 완성은 완료되었고, 완전 이식 여부는 M21~M52 전수 게이트로만 닫는다.
...
- M20 blocker 중 특수 item 15개는 M30에서, 신체/능력/소질/경험 seed는 M33에서, CFLAG 정의 151개와 Chara CFLAG seed 1,465개 및 RELATION seed 532개는 M34에서, 턴/시간 진행 row는 M35에서, 방문/시설 row는 M36에서, 업무/창관/특수 업무 row는 M37에서, 촬영 장면 정의 row는 M38에서 소비/분류를 닫았다. 남은 큰 묶음은 촬영 실행/훈련/미션/이벤트/엔딩/정보 계열이며 M39~M49에서 의미별 owner와 lifecycle로 분해해야 한다.
...
- M38에서 ERB-derived 촬영 장면 6개를 runtime `definitions.filmingSceneDefinitions`로 연결했다. 장면/대상 선택값과 촬영량 버퍼는 `GameSession.shooting`에만 남고, 촬영 실행 결과 저장은 M39 소유로 남긴다.
- 다음 작업은 M39이다. M39 placeholder script를 실제 `coverage:filming-execution`, `gate:filming-execution`, `smoke:filming-all` 구현으로 교체하고, 촬영 실행 결과/수익/팬/점수/이력/출시/판매/session cleanup/save roundtrip을 coverage와 gate로 닫는다.
